...
<?php //echo "Hi"; print_r($admin_data); ?>
<style type="text/css">
<?php if($admin_data->weekly_offs === 'Monday'){ ?>
    .fc-mon { 
                color:black;
                background-color: #f9bdbd!important;
            }
<?php }elseif($admin_data->weekly_offs === 'Tuesday'){ ?>
    .fc-tue { 
                color:black;
                background-color: #f9bdbd!important;
            }
<?php }elseif($admin_data->weekly_offs === 'Wednesday'){ ?>
    .fc-wed { 
                color:black;
                background-color: #f9bdbd!important;
            }
<?php }elseif($admin_data->weekly_offs === 'Thursday'){ ?>
    .fc-thu { 
                color:black;
                background-color: #f9bdbd!important;
            }
<?php }elseif($admin_data->weekly_offs === 'Friday'){ ?>
    .fc-fri { 
                color:black;
                background-color: #f9bdbd!important;
            }
<?php }elseif($admin_data->weekly_offs === 'Saturday'){ ?>
    .fc-sat { 
                color:black;
                background-color: #f9bdbd!important;
            }
                        ...
                        <div class="x_title">
                            ...
                            <form action="<?php echo base_url('backend/add_attendance'); ?>" method="post" name="add_attendance" id="add_attendance">
                                ...
                                <div class="col-md-4" style="float: right;">
                                    <span class="wOff margin-5">Week Off</span>
                                    <span class="typeLeave margin-5">Leave</span>
                                    <span class="typeHoliday margin-5">Holiday</span>
                                    <span class="typeOverDay margin-5">Over Day</span>
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="col-md-4">
                                    <label for="Leave">
                                        <input type="radio" name="leave_type" id="Leave" value="Leave" checked/>Leave
                                    </label>
                                </div>
                                <div class="col-md-4">
                                    <label for="Over_day">
                                        <input type="radio" name="leave_type" id="Over_day" value="Over day" />Over day
                                    </label>
                                </div>
                                <div class="col-md-4">
                                    <label for="Holiday">
                                        <input type="radio" name="leave_type" id="Holiday" value="Holiday" />Holiday
                                    </label>
                                </div>
                            </div>
                        </div>
...
$(function(){
    ...
    var selectableOrNot = '<?php if($admin_id != 0){ echo true; }else{ echo false; }?>';
    /*alert(selectableOrNot);*/
    ...
    $('#my-calendar').fullCalendar({
        ...
        eventSources: [
            {
              url: base_url+'backend/getAdminLeaveDetailsById',
              method: 'POST',
              data: {
    ...
    function modal(data) {
        ...
        if(data.event)
        {
            ...
            if(data.event.leave_type == 'Leave'){
                $('#Leave').prop('checked', 'checked');
            }else if(data.event.leave_type == 'Over day'){
                $('#Over_day').prop('checked', 'checked');
            }else{
    ...
    $('.modal').on('click', '#add-leave-details',  function(e){ 
        if(validator(['description'])) { 
            $.post(base_url+'backend/addAdminLeaveDetails', { 
                employee_id: $('#admin_id').val(),
                description: $('#description').val(),
                leave_type: $('input[name=leave_type]:checked').val(),
                start: $('#start').val(),
                end: $('#end').val()
            }, function(result){
                $('.alert').addClass('alert-success').text('Leave Details added successfuly');
                $('.modal').modal('hide');
    ...
    $('.modal').on('click', '#update-leave-details',  function(e){
        if(validator(['description'])) {
            $.post(base_url+'backend/updateAdminLeaveDetails', {
                id: currentEvent._id,
                description: $('#description').val(),
                leave_type: $('input[name=leave_type]:checked').val(),
            }, function(result){
    ...
    $('.modal').on('click', '#delete-event',  function(e){
        $.get(base_url+'backend/deleteAdminLeaveDetails?id=' + currentEvent._id, function(result){
    ...
    function validator(elements) {
        ...
        if(errors) {
            $('.error').html('Please add leave details ') ;
            return false;
        }
        return true;
    }
